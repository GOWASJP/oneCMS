...
行政・自治体・コーポレートサイト向けに、従来の動的CMS（ローカル起動→静的書き出し）の重い運用を置き換えます。
...
左サイドバーのナビゲーション + クリーンな編集エリアの、シンプルで見やすい管理画面です。
...
- 進捗バー付き・UIが固まりにくい書き出し（大量ページでも処理中に画面が応答）
- 検索インデックスを言語別に分割生成（search-index.{lang}.json／訪問者は自言語分のみ読み込み）
- 検索ページ（クライアントサイド全文検索）を言語別に自動生成
- アセットファイルの自動コピー
